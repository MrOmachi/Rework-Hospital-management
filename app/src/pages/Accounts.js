...
import {
  showError,
  showSuccess,
} from "../redux/slices/linkedAccounts/linkedAccountSlice";
...
function Accounts(props) {
  ...
  const linkAcctErrorMsg = useSelector(
    (state) => state.linkedAccount.linkAcctErrorMsg
  );
  const linkAcctSuccessMsg = useSelector(
    (state) => state.linkedAccount.linkAcctSuccessMsg
  );

  const onExit = useCallback(
    createOnExit(
      () => dispatch(fetchToken()),
      (err) => dispatch(showError("An error occured, please retry!"))
    ),
    []
  );
